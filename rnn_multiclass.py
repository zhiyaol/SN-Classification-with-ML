...
def _rnn_model_fn(features, labels, num_classes, hidden_units, optimizer, activation_fn,
    ...
    logits = slim.fully_connected(logits, num_classes, scope='fc/fc_3',  activation_fn=None)
    ...
    loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf.cast(labels, tf.float32), logits=logits))
...
class RNNClassifier(tf.estimator.Estimator):
    ...
    def __init__(self,
        ...
        def _model_fn(features, labels, mode):
            return _rnn_model_fn(features, labels, num_classes, hidden_units, optimizer, activation_fn,
                              normalizer_fn, dropout, mode)
